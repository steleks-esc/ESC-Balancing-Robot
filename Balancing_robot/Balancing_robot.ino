...
void setup(){
  ...
  pinMode(10, OUTPUT);                                                       //Configure digital poort 2 as output
  pinMode(7, OUTPUT);                                                       //Configure digital poort 3 as output
  pinMode(9, OUTPUT);                                                       //Configure digital poort 4 as output
...
ISR(TIMER2_COMPA_vect){
  ...
  else if(throttle_counter_left_motor == 1)PORTB |= B00000010;             //Set output 2 high to create a pulse for the stepper controller
  else if(throttle_counter_left_motor == 2)PORTB &= B11111101;             //Set output 2 low because the pulse only has to last for 20us 
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           //If the number of loops is larger then the throttle_right_motor_memory variable
    ...
    if(throttle_right_motor_memory < 0){                                    //If the throttle_right_motor_memory is negative
      PORTD |= 0b10000000;                                                  //Set output 5 low to reverse the direction of the stepper controller
      throttle_right_motor_memory *= -1;                                    //Invert the throttle_right_motor_memory variable
    }
    else PORTD &= 0b01111111;                                               //Set output 5 high for a forward direction of the stepper motor
  }
  else if(throttle_counter_right_motor == 1)PORTB |= B00000100;            //Set output 4 high to create a pulse for the stepper controller
  else if(throttle_counter_right_motor == 2)PORTB &= B11111011;            //Set output 4 low because the pulse only has to last for 20us
}
